...
export default function VehicleDetailsModal({ vehicle, isOpen, onClose, onBook }: VehicleDetailsModalProps) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div key={`vehicle-details-modal-overlay-${vehicle.id}`} className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto scroll-smooth">
          <motion.div
          ...
          <motion.div
            ...
            className="relative bg-dark-2 w-full max-w-6xl rounded-[14px] shadow-2xl overflow-y-auto md:overflow-hidden flex flex-col md:flex-row max-h-[95vh] md:max-h-[85vh] border border-gold/15 custom-scrollbar scroll-smooth"
          >
            <button
              onClick={onClose}
