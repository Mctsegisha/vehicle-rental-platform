...
export default function Home({ user, onAuthClick }: HomeProps) {
  ...
  const heroVehicle = featuredVehicles.length > 0 ? featuredVehicles[0] : null;
  ...
  return (
    <div className="bg-dark space-y-24 pb-24">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 items-center gap-16">
            ...
            >
              <div 
                ...
                className="aspect-[16/10] bg-dark-3 rounded-[14px] border border-gold/15 flex items-center justify-center relative overflow-hidden group cursor-pointer transition-all"
              >
                ...
                <div className="absolute bottom-6 left-6 right-6 p-4 bg-dark-2/90 backdrop-blur-md border border-white/5 rounded-lg flex items-center justify-between z-20">
                  <div>
                    <p className="text-[10px] font-bold text-gold uppercase tracking-widest mb-0.5">Featured Model</p>
                    <p className="text-sm font-bold text-white">{heroVehicle ? heroVehicle.name : 'Toyota Land Cruiser V8'}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-muted font-bold uppercase tracking-widest mb-0.5">Starting at</p>
                    <p className="text-sm font-bold text-gold">ETB {heroVehicle ? heroVehicle.pricePerDay.toLocaleString() : '2,500'}/day</p>
                  </div>
                </div>
              </div>
              {/* Decorative elements */}
      ...
      {heroVehicle && (
        <VehicleDetailsModal
          vehicle={heroVehicle}
          isOpen={isHeroDetailsOpen}
          onClose={() => setIsHeroDetailsOpen(false)}
          onBook={handleBook}
        />
      )}
    </div>
